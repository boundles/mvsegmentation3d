...
class SparseBottleneck(spconv.SparseModule):
    def __init__(self, inplanes, planes, stride=1, expansion = 4, downsample=None, indice_key=None, norm_fn=None, act_fn=None):
        super(SparseBottleneck, self).__init__()

        self.conv1 = spconv.SubMConv3d(
            inplanes, planes, kernel_size=1, stride=stride, padding=1, bias=False, indice_key=indice_key
        )
...
class SparseUnet(nn.Module):
    ...
    def __init__(self, input_channels, grid_size, voxel_size, point_cloud_range):
        ...
        self.conv1 = spconv.SparseSequential(
            block(32, 32, 3, norm_fn=norm_fn, act_fn=act_fn, padding=1, indice_key='subm1'),
            SparseBottleneck(32, 32, norm_fn=norm_fn, act_fn=act_fn, indice_key='subm1')
        )
