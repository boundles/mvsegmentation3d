...
class WaymoDataset(Dataset):
    def __init__(self, cfg, root, split='training', test_mode=False):
        ...
        for filename in self.filenames:
            ...
            self.idx_to_name_map[(file_idx, frame_idx)] = filename
    ...
    def get_lidar_from_sweeps(self, filename, num_sweeps=3, max_num_sweeps=10, pad_empty_sweeps=False):
        # current frame
        file_idx, timestamp, frame_idx = self.parse_info_from_filename(filename)
        ...
        ts = timestamp / 1e6
        transform_matrix = self.get_pose(filename)
        ...
        if pad_empty_sweeps and len(sweep_filenames) == 0:
            ...
            for idx in choices:
                ...
                points_sweep = self.get_lidar(sweep_filename)
                timestamp = self.parse_info_from_filename(sweep_filename)[1]
                sweep_ts = timestamp / 1e6
                sweep_transform_matrix = self.get_pose(sweep_filename)
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.USE_MULTI_SWEEPS:
            points = self.get_lidar_from_sweeps(filename, self.cfg.DATASET.NUM_SWEEPS)
        else:
            points = self.get_lidar(filename)
        input_dict = {
