...
from mvseg3d.datasets.transforms.test_time_aug import MultiScaleFlipAug
...
def semseg_for_one_frame(model, data_dict, transforms):
...
def inference(args, data_loader, model, logger):
    ...
    transforms = MultiScaleFlipAug(scales=[0.95, 1.0, 1.05], angles=[-1.57079, 0, 1.57079, 3.14159],
    ...
    for step, data_dict in enumerate(tqdm(data_loader), 1):
        segmentation_frame = semseg_for_one_frame(model, data_dict, transforms)
